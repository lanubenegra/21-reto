...
const PROVIDER_META: Record<string, { label: string; helper?: string; icon: ReactNode }> = {
  google: {
    label: "Google",
...
export default function SignInPage() {
  ...
  const socialButtons = oauthProviders.map(provider => {
    const meta = PROVIDER_META[provider.id] ?? { label: provider.name, icon: <ShieldCheck className="h-5 w-5" /> };
    ...
    return (
      <button
        ...
        className="flex h-12 w-12 items-center justify-center rounded-full border border-white/15 bg-white/12 text-white transition hover:bg-white/20"
        aria-label={`Continuar con ${meta.label}`}
      >
        {isLoading ? <span className="text-xs uppercase tracking-[0.35em]">...</span> : meta.icon}
      </button>
    );
  });
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-xl items-center justify-center px-6 py-16">
        <div className="w-full max-w-md rounded-[32px] border border-white/15 bg-[#0b1635]/95 p-9 shadow-[0_40px_90px_-35px_rgba(5,9,27,0.85)]">
            ...
            <div className="mt-6 space-y-5">
              ...
              {oauthProviders.length > 0 && (
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-3">{socialButtons}</div>
                </div>
              )}
            </div>
          )}
          ...
          <div className="mt-8 flex items-center justify-between text-xs text-white/60">
            <Link href="/" className="text-white underline">Volver al inicio</Link>
            <span>Protegemos tus datos con NextAuth</span>
          </div>
        </div>
      </div>
    </div>
